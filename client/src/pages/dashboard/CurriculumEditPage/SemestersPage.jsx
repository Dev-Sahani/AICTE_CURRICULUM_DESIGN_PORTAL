...
import { AddSubject, Table, CardWrapper, Loading } from "../../../components";
  ...
  if (localLoading) {
    return (
      <>
        <Loading
          count={4}
          containerClassName="m-10 pt-12 p-8 bg-gray-100 rounded-3xl"
          cardClassName="!h-12 rounded-3xl"
        />
        <Loading
          count={4}
          containerClassName="m-10 pt-12 p-8 bg-gray-100 rounded-3xl"
  ...
  return (
    <>
      {Object.keys(semesters || {}).map((semester, index) => {
        const totalCredits = semesters[semester].reduce((credits, item) => {
        ...
        return (
          <CardWrapper classNames="m-8 p-6 pt-2" key={index}>
            <header className="mx-2 flex justify-between items-center text-gray-900">
              <h1 className="inline text-base">{`${index + 1}.`}</h1>
              <h1 className="inline text-2xl-custom font-semibold">
                {`Semester-${semester}` || "-----"}
              </h1>
              <h1 className="inline text-base">{`Credits-${totalCredits}`}</h1>
            </header>
            ...
            <div className="mt-4 w-full flex justify-end text-white">
              <button
                className="bg-secondary-500 py-1.5 px-2.5 rounded"
                onClick={() => {
