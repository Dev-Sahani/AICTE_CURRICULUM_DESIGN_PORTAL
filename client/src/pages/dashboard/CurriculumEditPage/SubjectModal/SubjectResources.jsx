...
export default function SubjectResource() {
  ...
  if(localLoading) {
    return (
      <Loading count={5} cardClassName="h-20 w-full" containerClassName="w-full mt-12" />
    )
  }
  ...
  <div className="mt-2 flex flex-col p-2 gap-4">
    <div className="w-full px-2 flex justify-between items-center">
      <h1 className="text-primary-500 text-2xl font-medium">Resources</h1>
...

    {
      ...
      <div>
        ...
        <h1 className="pb-2 ml-2 text-primary-500 text-xl font-medium">
      ...
      {
        addMaterial && 
        <AddMaterial onClose={()=>setAddMaterial(false)} handleAdd={handleAdd}/>
      }
  </div>
  )
}
...
function Resource({ resource, isAdd, handleDelete, handleAccept, del}) {
  const classOne = classNames("flex flex-row gap-4 items-center rounded-md p-3 h-full",{
    "bg-accent-100": isAdd,
    "bg-primary-100": !isAdd,
    "!bg-red-100": del
  })

  return <div className='mx-1 flex flex-col w-full min-w-fit'>
        ...
        <div className='flex flex-col gap-4'>
          <div className='flex flex-row justify-between'>
            <div className='flex flex-row space-x-2 '>
              <div className='rounded-2xl bg-[#FEDEEA] px-4 py-1 text-sm font-text-[#F8186E] items-center justify-center'>
                {resource?.title}
              </div>
          ...
          <div className='ml-2 text-sm'>by  {resource?.author}</div>
        </div>
      </div>
      {
        (isAdd || del)?
        <button onClick={handleAccept} className={`px-3 py-1.5 rounded ${del ? "bg-red-400" : "bg-secondary-500"} text-white`}>
            Save
        </button>
        :
