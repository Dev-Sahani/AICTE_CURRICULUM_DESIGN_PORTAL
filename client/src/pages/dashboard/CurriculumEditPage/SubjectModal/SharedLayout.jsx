...
export default function SharedLayout({ className, children, currentPath }) {
  ...
  if (localLoading) {
    return (
      <div>
        <div>
          <Loading count={1} cardClassName="!h-12" />
        </div>
        <div className="flex">
  ...
  return (
    <div className={"flex flex-col h-full w-full p-2 " + className}>
      ...
      <header className="m-2 flex justify-between items-end w-full ">
        <h1 className="ml-2 inline text-xl-custom text-gray-500">
          {data?.code}
        </h1>
        <h1 className="inline text-2xl-custom text-primary-500 font-semibold">
          {data?.title}
        </h1>
        <h1 className="mr-16 inline text-xl-custom text-gray-500">
          {data?.credits}C
        </h1>
      </header>
        ...
        <nav className="flex flex-col w-40">
          <ul className="flex flex-col gap-2 px-2 pt-1 text-center text-white">
            <Link to="">
              <li
                className={`${
                  !["syllabus", "resources"].some((char) =>
                    currentPath.endsWith(char)
                  )
                    ? "bg-primary-700"
                    : "bg-primary-500"
