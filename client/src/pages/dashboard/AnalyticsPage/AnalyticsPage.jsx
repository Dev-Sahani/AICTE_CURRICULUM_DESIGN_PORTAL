...
const AnalyticsPage = ()=>{
  ...
  const search = useCallback(async ()=>{
    return await axios.get(`${BASE_URL}/api/v1/feedback/analysis/${course?.common_id}`,{
  ...
  useEffect(()=>{
    if(course?.common_id){
      ...
      search()
        .then((response)=>{
          response.data?.data.sort((a,b)=>a.fields?.questionNo - b.fields?.questionNo)
    ...
    <SearchBar setCourse={setCourse}/>
    {
      ...
      <div>
        ...
        <header className="my-8 w-full">
          <h1 className="mt-4 mx-2 text-3xl text-primary-600 font-bold">{course.title?.cur}</h1>
          <h2 className="mx-2 inline text-xl text-secondary-500">{course.level?.cur}</h2>
          <h2 className="mb-4 mx-2 inline text-xl text-secondary-500">{course.program?.cur}</h2>
        </header>
        ...
        <div className="w-full grid grid-cols-[1fr_1.5fr] gap-x-8 gap-y-10">
          ...
          <div className="bg-white shadow-lg rounded-xl p-4 grid grid-cols-[1fr_1fr] grid-rows-[1.5fr_1fr] gap-6">
            <PieChart className="row-start-1 row-end-3" yesPer={data[1]?.integersValues*20} width="180px"/>
            <p className="font-medium">
              <span className="text-accent-500">{(data[1]?.integersValues *20).toFixed(2)}%</span> people like this course curriculum
            </p>
            <p className="font-medium">
              <span className="bg-primary-600 inline-block w-4 aspect-square mr-2" /> Yes
              <br />
              <span className="bg-primary-200 inline-block w-4 aspect-square mr-2" /> No
            </p>
          </div>
