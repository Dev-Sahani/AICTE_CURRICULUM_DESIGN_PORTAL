...
const CourseDropdown = ({ name = "", list = [], subjectId }) => {
  ...
  if (
    ...
    propertyName.cur &&
    propertyName !== "" &&
    !list.includes(propertyName.cur)
  )
  ...
  return (
    <div className="flex items-center">
      <select
        ...
        value={value}
        onChange={onChange}
        className={`border-2 border-gray-300 m-2 px-4 py-1 focus:outline-none ${
          propertyName && propertyName.new && propertyName.new.length > 0
            ? "bg-accent-300 rounded-l border-r-0 mr-0"
            : "rounded"
        }`}
      >
      ...
      {propertyName && propertyName.new && propertyName.new.length > 0 && (
        // <button className="text-sm-custom border-2 border-gray-400 h-full w-full rounded-r-lg overflow-hidden">
        <ViewChangesButton
          className="!p-0 !rounded-l-none border-2 border-gray-300"
          name={name}
          showImage
          imageClassName="w-[1.92rem]"
        />
      )}

      {showMessage && (
        <Modal
          className="!w-[20rem] !h-[10rem] p-4 py-6 text-center flex flex-col justify-between"
          onClose={handleCancel}
        >
          <p>{`Are you really want to change the value of ${name}`}</p>
          <div className="w-full flex items-center justify-around">
            <button
              className="min-w-[100px] px-2 py-1 rounded bg-secondary-500"
              onClick={handleSubmit}
            >
              Yes
            </button>
            <button
              className="min-w-[100px] px-2 py-1 rounded bg-red-400"
