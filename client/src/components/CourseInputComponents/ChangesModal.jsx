...
export default function ChangesModal({ name, subName, onClose }) {
  ...
  const { [propertyName]: actualPropertyValue, acceptChanges } =
    useCourseContext();
  const {
    user: { role },
  } = useUserContext();
  ...
  if (
    ...
    index * 1 < propertyValue.cur.length &&
    propertyValue.cur[index * 1].cur &&
    propertyValue.cur[index * 1].cur[subName]
  ) {
    propertyValue = {
      cur: propertyValue.cur[index * 1].cur[subName],
      new: propertyValue.cur[index * 1].new?.reduce(
        (currSub, newSub, newSubIndx) =>
          newSub?.value[subName]
            ? [
                ...currSub,
                {
                  by: newSub.by,
                  value: newSub.value[subName],
                  index: newSubIndx,
                },
              ]
            : currSub,
  ...
  if (
    (propertyValue?.new &&
      propertyValue.new[propertyValue.new.length - 1]?.value) !== "deleted" &&
    !subName
  ) {
    actualPropertyValue?.del?.forEach((item, index) => {
      if (item?.index === index * 1)
        propertyValue?.new?.push({
          by: item.by,
          value: "deleted",
          index: index,
        });
    ...
    <Modal onClose={onClose} className="!w-[56rem]">
      <div className="h-full p-6 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="w-full flex gap-4 items-start justify-between">
          <h3 className="mt-2 text-lg-custom font-medium">Current Value</h3>
          <div
            className={`basis-[85%] px-1.5 py-1 ${
              typeof propertyValue.cur !== "object" &&
              "max-h-56 border border-black bg-white rounded-md overflow-y-auto"
            } flex flex-col gap-2`}
          >
              ...
              ? Object.keys(propertyValue.cur).map((key) => {
                  return (
                    <div className="w-full flex justify-between" key={key}>
                      ...
                      <input
                        type="text"
                        className="w-full p-1.5 outline-none border-2 border-gray-300"
                        value={propertyValue.cur[key]}
                        onChange={() => {}}
                      />
                    </div>
                  );
        ...
        <div className="flex justify-between gap-4">
          <h3 className="mt-2 text-lg-custom font-medium">New Values</h3>
              ...
              propertyValue.new.map((item, ind) =>
                typeof item?.value === "object" ? (
                  <div className="w-full flex flex-col gap-2" key={ind}>
                    ...
                    <p className="w-full text-end text-gray-500">
                      by: {item.by}
                    </p>
                  </div>
                ) : (
                  <div
                    className={`mb-4 flex flex-col gap-1 items-end`}
                    key={ind}
                  >
                    <div
                      className={`max-h-56 w-full px-1.5 py-1 border border-black ${
                        item?.value === "deleted"
                          ? "bg-red-400 text-white text-center capitalize"
                          : "bg-white"
