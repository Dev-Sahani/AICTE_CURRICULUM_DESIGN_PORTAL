  ...
  return (
    <Modal onClose={onClose} className={modalClassName}>
      <form
        ...
        className="h-full w-full p-4 flex flex-col gap-2 justify-between"
        ref={formRef}
      >
        <h1 className="w-full text-center text-3xl text-semibold text-primary-500">
          Enter basics details
        </h1>
        <div className="w-full flex flex-col gap-2">
          {inputFields.map((input, ind) => {
            ...
            if (
              ...
              inputBox = (
                <input
                  className="p-1 rounded w-full border-2 border-gray-300"
                  name={input?.title}
                  required
                  {...input}
                />
            ...
            return (
              ...
              >
                {input?.title && (
                  <label className="font-medium text-lg-custom capitalize min-w-[120px]">
                    {input?.title}
                  </label>
                )}
                {inputBox}
        ...
        */}
        <div className="w-full flex gap-4 justify-end items-center">
          <button
            type="submit"
            className="bg-secondary-500 py-1.5 px-2.5 text-white rounded"
          >
            Create
          </button>
          <button
            onClick={onClose}
            className="py-1.5 px-2.5 rounded text-white bg-red-500"
          >
            Cancel
          </button>
        </div>
