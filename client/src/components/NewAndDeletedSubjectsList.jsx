...
export default function NewAndDeletedSubjectsList({deleted=false}) {
  const { subjects: { add: newSubjects, del: deletedSubjects, cur: subjects}, acceptChanges} = useCourseContext();
  ...
  const handleAccept = async (index) => {
    await acceptChanges("subjects", index, !deleted, deleted, !deleted ? arrayOfSubject[index]?.value?.title : undefined);
  }
