...
export default function Table({
  ...
  if (!data || !keys || !Array.isArray(keys))
    return <div>Data is not passed</div>;

  const rowClasses =
    "w-full py-2 px-2 lg:px-6 xl:px-10 text-center bg-accent-400";
  ...
  const handleDelete = async (subjectId) => {
    ...
    subjects?.cur?.find((sub, indx) => {
      if (sub?.cur?.common_id === subjectId) actualIndex = indx;
        ...
        return (
          <div
            className={`flex flex-col ${
              value === "-" ? "w-fit items-start" : "w-full items-center"
          ...
          >
            <header
              className={`${
                secondaryHeader
                  ? `text-center text-white bg-primary-700 p-2`
                  : "text-primary-500"
              } ${
                value === "-" ? "w-fit px-5" : "w-full"
              } text-lg-custom font-semibold capitalize`}
            >
              <h2
                className={`${value === "-" && "text-transparent"} text-center`}
              >
                {value}
              </h2>
            </header>
              ...
              {data?.map((item, ind) => {
                ...
                return value === "-" ? (
                  <button
                    className={classes}
                    key={ind}
                    onClick={() => handleDelete(item?.cur?.common_id)}
                  >
                    <img
