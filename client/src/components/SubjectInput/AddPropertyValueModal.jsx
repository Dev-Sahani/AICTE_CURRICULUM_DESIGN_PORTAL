...
export default function AddPropertyValueModal({propertyName, setShowModal}) {
  ...
  return (
    <Modal onClose={()=>setShowModal("")} className="!h-[16rem] !w-[28rem] py-4 px-8 flex flex-col justify-evenly items-center">
      <h2 className="text-primary-500 text-xl font-bold">Enter value of the field:</h2>
        {
          localLoading ?
      ...
      <div className="w-full flex gap-2 justify-evenly">
        <button className="px-2 py-1 rounded bg-secondary-500 text-white" onClick={handleAdd} disabled={localLoading}>
          Add +
        </button>
        <button className="px-2 py-1 rounded bg-red-500 text-white" onClick={()=>setShowModal("")} disabled={localLoading}>
          Cancel
        </button>
      </div>
    </Modal>
